...
// `event.req` is h3 v2's ServerRequest at runtime (it extends Request), but
// Nuxt's server tsconfig pins `h3` types to v1 where event.req is IncomingMessage.
export default defineEventHandler(event => useAuth().handler(event.req as unknown as Request))
