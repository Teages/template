...
/**
 * Port file written by `nitroTestPlugin` when it binds a localhost proxy in front
 * of `environments.nitro.dispatchFetch` (env-runner).
 *
 * Tests cannot share `globalThis` or module state with the Vite host process /
 * ModuleRunner split, so we bridge over loopback HTTP instead.
 */
