...
### CJS-only entrypoints inlined into the ModuleRunner
- reason: `@whatwg-node/fetch` (a `graphql-yoga` dependency) exposes a CJS
...
- affected: dev, test
- patched: `plugins/module-runner-esm/index.ts` resolves `@whatwg-node/fetch`
...
- follow up: Drop the remap when upstream ships an env-runner-compatible ESM
  entrypoint.
