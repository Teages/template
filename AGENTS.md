...
`index.html` is the entry point at the project root. `app/` is the Vue frontend with `entry-client.ts`, `entry-server.ts`, `app.vue`, plus `pages/`, `components/`, `composables/`, `utils/`, and `assets/`. `server/` contains Nitro server code under the supported subdirs: `api/` (`/api`-prefixed handlers for REST, GraphQL, tRPC, and auth), `middleware/`, `plugins/`, `utils/`, `tasks/`, and the domain dirs `rest/`, `graphql/`, `trpc/`, and `database/`. `plugins/` holds local Vite plugins. `test/` is split into `unit/` and `e2e/`. `public/` holds static assets. Config files: `vite.config.ts` (loads `nitro/vite`, configures auto-imports via the `@nuxt/ui` Vite plugin), `nitro.config.ts`, `drizzle.config.ts`, and tsconfig files extending nitro/tsconfig with the `~/*` path alias.
